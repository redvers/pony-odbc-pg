...
actor Main is TestList
  ...
  fun tag tests(test: PonyTest) =>
    test(_DBConnect)
    test(_DBConnectFail)
    test(_DBConnectMultiple)



class iso _DBConnect is UnitTest
...
class iso _DBConnectFail is UnitTest
...
class iso _DBConnectMultiple is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    h.assert_false(dbc.connect("psqlred"))
    h.assert_false(dbc.is_valid())
